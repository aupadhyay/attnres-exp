...
def plot_cosine_similarity(vectors: np.ndarray, output_path: str, title: str = "", label_prefix: str = "L"):
        ...
        for j in range(n):
            if not mask[i, j]:
                ax.text(j, i, f"{cos_sim[i, j]:.2f}", ha="center", va="center",
                        color="white" if abs(cos_sim[i, j]) > 0.5 else "black", fontsize=10)
    ax.set_xticks(range(n))
    ax.set_yticks(range(n))
    ax.set_xticklabels([f"{label_prefix}{i+1}" for i in range(n)])
    ax.set_yticklabels([f"{label_prefix}{i+1}" for i in range(n)])
...
def plot_pca(vectors: np.ndarray, output_path: str, title: str = "", label_prefix: str = "L"):
    ...
    for i in range(n):
        ax.annotate(f"{label_prefix}{i+1}", (projected[i, 0], projected[i, 1]),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label-prefix", default="L", help="Label prefix: L for layers, B for blocks")
